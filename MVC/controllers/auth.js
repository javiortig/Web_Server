...
/**
 * Encargado de modificar un rol de usuario a admin
 * @param {*} req 
 * @param {*} res 
 */
const makeAdminCtrl = async (req, res) => {
    try{
        req = matchedData(req)
        const user = await usersModel.findOneAndUpdate({ email: req.userEmail }, 
            { $set: { role: "admin" } })

        // TODO: Envia el usuario pero enseña su antiguo rol antes de cambiar a admin
        res.send(user)


    }catch(err){
        console.log(err)
        handleHttpError(res, "ERROR_MAKING_ADMIN")
    }
}

module.exports = { registerCtrl, loginCtrl, makeAdminCtrl }